...
internal sealed class HelpDialog : Form
{
    public HelpDialog(Action<string>? openRecommendation = null)
    {
        ...
        steps.Controls.Add(Step("1", "🔗 Добавьте подписку", "Нажмите «Настроить», один раз вставьте полученную у VPN-провайдера HTTPS-ссылку и продолжите."));
        steps.Controls.Add(Step("2", "⚡ Дождитесь проверки", "Автовыбор проверит доступ к сервисам, найдёт рабочий канал и до двух резервов из доступных узлов. Интернет пока не переключается."));
        steps.Controls.Add(Step("3", "▶ Выберите режим", "«Авто» — встроенный канал и проверенный резерв. «ART VPN» — только встроенный канал. «Throne» и «HAPP» — ручной выбор настроенного клиента. «Вернуть прежнее» восстановит прежний прокси."));
